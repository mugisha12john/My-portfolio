...
const FACTS = [
  { label: "Location", value: "Kigali, Rwanda", colorClass: "text-text" },
  { label: "Football", value: "Arsenal · Apr Fc", colorClass: "text-text" },
  { label: "Languages", value: "RW · FR · EN", colorClass: "text-text" },
  { label: "Status", value: "Open to Work", colorClass: "text-green" },
];
          ...
          <div className="about-facts-row grid grid-cols-2 gap-px bg-[rgba(127,238,100,0.12)] border border-[rgba(127,238,100,0.12)] mt-4.5">
            {FACTS.map((fact, i) => (
              <div key={i} className="fact bg-bg-2 p-3 px-3.5">
                ...
                <div
                  className={`fact-value text-[12px] font-semibold ${fact.colorClass}`}
                >
                  {fact.value}
                </div>
              </div>
